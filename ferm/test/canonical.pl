...
while (<>) {
    ...
    if (s/^iptables //) {
        ...
        if ($command eq 'F' or $command eq 'X') {
            if (defined $chain) {
                delete $data->{iptables}{$table}{$chain}{rules};
            } else {
                delete $data->{iptables}{$table};
            }
            next;
        } elsif ($command eq 'N') {
            push @{$data->{iptables}{$table}{$chain}{rules}}, 'NEW';
            next;
        }
        ...
        s/(!\s*)?--(syn|clamp-mss-to-pmtu|set|rcheck|log-tcp-sequence|log-tcp-options|log-ip-options|continue)(\s|$)/$item->{$2} = $1; ''/eg;
