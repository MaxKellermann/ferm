...
language, which allows variables, functions, arrays, and blocks. It also
...
This manual page does I<not> intend to teach you how firewalling works
...
configuration files. Besides these special characters, ferm
...
Set the domain.  "ip" means "IPv4" (iptables) and "ip6" is for IPv6
support, using "ip6tables".  The default is the one specified with
B<--domain> or "ip" if the option was not specified.

=item B<table [filter|nat|mangle]>

Specifies which netfilter table this rule will be inserted into:
...
Works like B<@subchain> except that instead of using B<jump> target it
uses B<goto> target. See discussion below for the difference between
these two targets.
...
With this option, B<ferm> loads the previous rule set using
B<iptables-save>, extracts all "preserved" chains and inserts their
data into the output.

"Preserved" chains must not be modified with B<ferm>: no rules and no
policies.
...
Instead of B<protocol>, you can also use the shortcut B<proto>.
...
Specify that the SYN flag in a TCP packet should be matched,
which are used to build new TCP connections. You can identify
incoming connections with this, and decide whether you want
...
Instead of B<module>, you can also use the shortcut B<mod>.
...
Go to a custom chain.  Unlike the B<jump> option, B<RETURN> will not
...
Finish the current chain and return to the calling chain (if "jump
...
The path is relative to the root of the cgroupsv2 hierarchy and is compared
...
Valid values for I<connbytes-dir>: I<original>, I<reply>, I<both>; for
I<connbytes-mode>: I<packets>, I<bytes>, I<avgpkt>.
...
address in Ethernet frame with the lower 64 bits of the IPv6 source
...
Matches packets based on their geological location. (Needs an installed
GeoDB.)
...
This option can also be used in be I<ip6> domain, although this is
called B<icmpv6> in F<ip6tables>.
...
Check the packet length.
    ...
    mod length2 length 128 layer3; # exactly 128 bytes
    mod length2 length 512:768 layer4; # range
    mod length2 length ! 256 layer5; # negated
...
Matches the mobility header (domain I<ip6>).
...
Add packet accounting (bytes + packets). See also the user space tool "nfacct".
...
("cmd-owner", "pid-owner" and "sid-owner" require special kernel
...
Matches the IPsec policy being applied to this packet.
...
See L<http://ipset.netfilter.org/> for more information.
...
Successor of B<nth> and B<random>, currently undocumented in the
iptables(8) man page.
...
Check if the time a packet arrives is in the given range.
...
Configure a simple cluster of nodes that share a certain IP and MAC
...
=item B<IPV4OPTSSTRIP>

Strip all the IP options from a packet.  This module does not take any
options.
...
Log all packets that match this rule in the kernel log. Be careful
...
Map a whole network onto another network in the B<nat> table.
...
Log packets over netlink; this is the successor of I<ULOG>.
...
Userspace queueing, the predecessor to B<NFQUEUE>.  All packets go to
queue 0.
...
Similar to SNAT, but a client is mapped to the same source IP for all
its connections.
...
=item B<SET [add-set|del-set] [setname] [flag(s)]>

Add the IP to the specified set. See L<http://ipset.netfilter.org/>
...
TCP 3-way handshake proxy: let the firewall handle the TCP 3-way handshake and
only establish a connection with the server socket once the client handshake has
finished.
...
Set the TCP packet Type Of Service bit to this value.  This will be
...
Since version 2.0, B<ferm> supports not only I<ip> and I<ip6>, but
also I<arp> (ARP tables) and I<eb> (ethernet bridging tables).  The
concepts are similar to I<iptables>.

=head2 arptables keywords

=over 8

=item B<source-ip>, B<destination-ip>

Matches the source or destination IPv4 address.  Same as B<saddr> and
B<daddr> in the I<ip> domain.

=item B<source-mac>, B<destination-mac>

Matches the source or destination MAC address.

=item B<interface>, B<outerface>
...
Operation code, for details, see iptables(8).
...
The keywords B<mangle-ip-s>, B<mangle-ip-d>, B<mangle-mac-s>,
B<mangle-mac-d>, B<mangle-target> may be used for ARP mangling.  See
iptables(8) for details.

=back

=head2 ebtables keywords

=over 8

=item B<proto>

Matches the protocol which created the frame, e.g. I<IPv4> or B<PPP>.
For a list, see F</etc/ethertypes>.

=item B<interface>, B<outerface>

Physical input and output interface.

=item B<logical-in>, B<logical-out>

The logical bridge interface.

=item B<saddr>, B<daddr>

Matches source or destination MAC address.

=item B<Match modules>

The following match modules are supported: 802.3, arp, ip, mark_m,
pkttype, stp, vlan, log.

=item B<Target extensions>

The following target extensions are supported: arpreply, dnat, mark,
redirect, snat.

Please note that there is a conflict between I<--mark> from the
I<mark_m> match module and I<-j mark>.  Since both would be
implemented with the ferm keyword B<mark>, we decided to solve this by
...
The current domain.  One of I<ip>, I<ip6>, I<arp>, I<eb>.
...
With a trailing pipe symbol, B<ferm> executes a shell command and
parses its output:

    @include "/root/generate_ferm_rules.sh $HOSTNAME|"

B<ferm> aborts, if return code is not 0.

=head2 Conditionals

The keyword B<@if> introduces a conditional expression:
...
There is also B<@else>:

    @if $condition DROP; @else REJECT;

Note the semicolon before the B<@else>.

It is possible to use curly braces after either B<@if> or B<@else>:
...
There is no B<@elsif>, use B<@else @if> instead.
...
The specified command is executed using the shell.  "pre" means run
the command before applying the firewall rules, and "post" means run
the command afterwards.  "flush" hooks are run after ferm has flushed
...
=head2 @defined($name), @defined(&name)
...
=head2 @resolve((hostname1 hostname2 ...), [type])
...
which can be used to begin a new firewall. This section contains more
...
seconds (see B<--timeout>).  This is useful for remote firewall
...
Show a brief list of available command-line options.
...
Fast mode is enabled by default since B<ferm> 2.0, deprecating this
option.

=item B<--slow>

Disable fast mode, i.e. run iptables(8) for every rule, and don't use
iptables-restore(8).
...
Handle only the specified domain and choose it as default domain for
rules where none is specified.  B<ferm> output may be empty if the
domain is not configured in the input file.

=item B<--def '$name=value'>
...
L<https://github.com/MaxKellermann/ferm/issues>

=head1 COPYRIGHT

Copyright 2001-2021 Max Kellermann <max.kellermann@gmail.com>,
Auke Kok <sofar@foo-projects.org> and various other contributors.
...
Max Kellermann <max.kellermann@gmail.com>, Auke Kok
