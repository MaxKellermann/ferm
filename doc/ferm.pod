...
Since verison 2.0, B<ferm> supports not only I<ip> and I<ip6>, but
...
Please note that there is a conflict between "--mark" from the mark_m
match module and "-j mark".  Since both would be implemented with the
ferm keyword "mark", we decided to solve this by writing the target's
name in uppercase, like in the other domains.  The following example
rewrites mark 1 to 2:
...
the command afterwards.  You may install any number of hooks.
...
This function currently only resolves B<A> records (i.e. IPv4
addresses), rendering it useless for the I<ip6> domain.
...
"source addr $A port $B" is deprecated, use "saddr $A sport $B"
...
Copyright (C) 2001-2008 Auke Kok <sofar@foo-projects.org>, Max
