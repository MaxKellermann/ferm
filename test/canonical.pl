...
while (<>) {
    ...
    if (s/^(ip6?)tables //) {
        ...
        s/(?:(!)\s*)?--(syn|clamp-mss-to-pmtu|set|rcheck|log-tcp-sequence|log-tcp-options|log-ip-options|continue|save-mark|restore-mark|save|restore|fragment|ecn-tcp-cwr|ecn-tcp-ece|physdev-is-(?:in|out|bridged)|strict|next|frag(res|first|more|last)|nodst|ecn-tcp-remove|ahres|soft|rt-0-res|rt-0-not-strict|ashort|new)(?:\s|$)/$item->{$2} = $1; ''/eg;
